...
int main()
{
    ...
    struct { char32_t cp; const char* name; } cases[] = {
    ...
    for (auto [cp, name] : cases)
    {
        ...
        if (w_console != 2 || w_wcswidth != 2 || w_unicode != 2 ||
            w_graphemes != 2 || w_g_ambig != 2)
            all_pass = false;
    }

    // ── Part 2: PS error msg full-string width ──
    std::cout << "=== Part 2: PS Error Message Full-String Width ===" << std::endl;
    std::u32string msg = U"wrong_command : 无法将\"wrong_command\"项识别为 cmdlet、函数、脚本文件或可运行程序的名称。请检查名称的拼写，如果包括路径，";
    ...
    for (size_t i = 0; i < msg.size(); ++i)
    {
        ...
        {
            char utf8[8] = {};
            if (cp < 0x80) { utf8[0] = static_cast<char>(cp); }
            else if (cp < 0x800) { utf8[0]=0xC0|(cp>>6); utf8[1]=0x80|(cp&0x3F); }
            else if (cp < 0x10000) { utf8[0]=0xE0|(cp>>12); utf8[1]=0x80|((cp>>6)&0x3F); utf8[2]=0x80|(cp&0x3F); }
            else { utf8[0]=0xF0|(cp>>18); utf8[1]=0x80|((cp>>12)&0x3F); utf8[2]=0x80|((cp>>6)&0x3F); utf8[3]=0x80|(cp&0x3F); }

            std::cout << std::setw(3) << i << "   U+" << std::hex << std::setw(4) << std::setfill('0')
                      << static_cast<uint32_t>(cp) << std::dec << std::setfill(' ')
                      << "   " << wc << "        " << ww << "         " << wu
                      << "      " << utf8 << std::endl;
        }
    }
