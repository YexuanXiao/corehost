...
[[nodiscard]] inline std::wstring query_process_command_line(win32::handle_view process) noexcept
{
    ...
    command_line.resize_and_overwrite(output_buffer_chars, [&](wchar_t *buffer, size_t capacity) noexcept {
        const auto status =
            ::NtQueryInformationProcess(process.get(), static_cast<PROCESSINFOCLASS>(ProcessCommandLineInformation),
                                        buffer, static_cast<ULONG>(capacity * sizeof(wchar_t)), nullptr);
        if (status < 0)
            return 0uz;
